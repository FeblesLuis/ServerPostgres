...
{
    public class BuscarUsuariosApellidoHandler : IRequestHandler<BuscarUsuariosApellidoQuery, List<BuscarUsuarioResponse>>
    {
        private readonly ApiDbContext _dbContext;
        private readonly ILogger<BuscarUsuariosApellidoHandler> _logger;
        public BuscarUsuariosApellidoHandler(ApiDbContext dbContext, ILogger<BuscarUsuariosApellidoHandler> logger)
        {
            _dbContext = dbContext;
            _logger = logger;
        }

        public Task<List<BuscarUsuarioResponse>> Handle(BuscarUsuariosApellidoQuery request, CancellationToken cancellationToken)
        {
            try
            {
        ...
        private async Task<List<BuscarUsuarioResponse>> HandleAsync(BuscarUsuariosApellidoQuery request)
        {

            try
            {
                _logger.LogInformation("ConsultarUsuarioapellidoQueryHandler.HandleAsync");
                var result = _dbContext.Usuario.Count(c => c.apellido == request._request.data);
                ...
                var usuarioId = _dbContext.Usuario.Where(c => c.apellido == request._request.data).Select(c => new BuscarUsuarioResponse() { id = c.Id, });
                ...
                //Traigo al usuario
                var usuario = _dbContext.Usuario.Where(c => c.apellido == request._request.data)
